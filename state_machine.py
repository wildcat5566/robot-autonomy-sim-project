	...
	while True:
		...
		if state == 'go_to_waypoint':
			...
			if waypoint == 'waypoint3':
				success,path = success,path = find_path(des_pose,quat,gripper=True,ignore_collisions=True)
				if success:
					execute_path(path)
					waypoint = 'waypoint4'
				else:
			...
			elif waypoint == "soup_grasp_point":
