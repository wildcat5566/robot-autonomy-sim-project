...
class RRT:
    ...
    def __init__(self, fr, is_in_collision):
        ...
        self._q_step_size = 0.08 #q2:0.09, q1:0.01
        self._target_p = 0.5 #q2:0.5 q1:0.3
    ...
    def sample_valid_joints(self):
        ...
        '''
        q = []
        for high, low in zip(self._fr.joint_limits_high, self._fr.joint_limits_low):
    ...
    def project_to_constraint(self, q, constraint):
        ...
        lr = self._project_step_size
        th = self._constraint_th
        err, grad = constraint(q_proj) #random larger value

        while err > th:
            jacobian = self._fr.jacobian(q_proj)
            q_proj -= lr * np.matmul(jacobian.T, grad)
    ...
    def extend(self, tree, q_target, constraint=None):
        ...
        found = False
        while not found: #loop until found a non-collision node
            q_sample = None
            pp = np.random.random()
            if pp < self._target_p:
                q_sample = q_target
            else:
            ...
            q_new = q_near + min(self._q_step_size, dist)*(q_sample - q_near) / dist
            ...
            if L2(q_new, q_target) < self._q_step_size:
                target_reached = True

            else:
                target_reached = False

        #3print("======L2======: {:.6f}".format(L2(q_new, q_target)))
